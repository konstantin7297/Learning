class UniversalInfo:
    ...
    web_server = {
        "info": "Для соединения Backend + Frontend, первый нужно запустить через спец. протокол (WSGI или AWSGI), который обеспечивает их совместимость.",
        "WSGI": "Синхронный протокол. Установка: 'pip install gunicorn'. Запуск: 'gunicorn --bind 0.0.0.0:5000 routes:app'. DEBUG: '--reload'.",
        "AWSGI": "Асинхронный протокол. Установка: 'pip install uvicorn'. Запуск: 'uvicorn routes:app --host 0.0.0.0 --port 5000'. DEBUG: '--reload'.",
...
class FastAPIApp:
    """ Класс содержит информацию касательно FastAPI. """
    # response_model - указывает тип сериализуемого объекта, который возвращает эндпоинт # noqa
    # FastAPI - сразу имеет встроенную документацию + Валидацию
    import uvicorn  # noqa
    from typing import List
    from fastapi import FastAPI, Request  # noqa
    from sqlalchemy import update  # noqa
    from sqlalchemy.future import select  # noqa
    from database import engine, Base, session, Recipes, AbstractModel  # noqa
    from schemas import FirstMonitor, SecondMonitor  # noqa

    app = FastAPI()

    def validation():  # noqa
        """ Тут написаны варианты валидации для FastAPI приложения """
        def schema_type():  # noqa
            """ Тут написаны схемы валидации для FastAPI приложения """
            from pydantic import BaseModel  # noqa

            class BaseBook(BaseModel):  # Общие параметры для всех валидаторов # noqa
                name: str
                time_to_create: int

            class FirstMonitor(BaseBook):  # Валидация к первому endpoint # noqa
                views: int

            class SecondMonitor(BaseBook):  # Валидация ко второму endpoint # noqa
                ingredients: str
                description: str

                class Config:  # Говорит, что мы пользуемся SQLAlchemy, конфиг поддерживается в pydantic # noqa
                    orm_mode = True

        def custom_type():  # noqa
            """ Тут показан пример создания кастомной валидации прямо внутри эндпоинта """  # noqa
            from fastapi import Path, Query, Body  # noqa
            from pydantic import Field, BaseModel  # noqa
            from typing import Optional

            def get():  # noqa
                @app.get('/hello/{who}')  # Валидация GET запроса # noqa
                async def say_hello(
                        who: int = Path(  # Path - для параметров прямо в url ссылке # noqa
                            ...,  # ... - значение не так уж важно или его нет, принимает тип из аннотации - int # noqa
                            title='Id of the user to whom to send the message.',  # Пригодится в документации # noqa
                            ge=0,  # id должен быть не меньше 0 # noqa
                            le=4,  # id должен быть не больше 4 # noqa
                        ),
                        message: Optional[str] = Query(  # Query - для параметров после знака "?" # noqa
                            None,
                            title="Say something pleasant to the user. Don't be arrogant (do not use uppercase)",
                            regex='^[a-z0-9_\-]+$'  # смотрит, что бы не было caps lock # noqa
                        )
                ):
            ...
            def post():  # noqa
                class Author(BaseModel):
                    name: str
                    born_year: int = Field(..., lt=2015)  # возраст автора должен быть меньше 2015 года # noqa
                ...
                class Book(BaseModel):
                    ...
                    author: Author
                    text: Optional[str] = None  # необязательное str значение # noqa

                @app.post('/books')  # noqa
                @app.post('/books/{idx}')  # пример составления валидации для post запроса # noqa
                async def post_book(
                        book: Book,
                        publisher: Optional[str] = Body(...),
                        idx: Optional[int] = None  # noqa
                ):
    ...
    @app.get('/cookbook', response_model=List[FirstMonitor])  # 1 Пример GET endpointa приложения # noqa
    async def cookbook() -> List[Recipes]:  # noqa
        res = await session.execute(select(Recipes.name, Recipes.views, Recipes.time_to_create).order_by(Recipes.views.desc()))  # noqa
        return res.fetchall()

    @app.get('/cookbook/{name}', response_model=SecondMonitor)  # 2 Пример GET endpointa приложения # noqa
    async def cookbook_details(name: str) -> Recipes:
        await session.execute(update(Recipes).where(Recipes.views == name).values(views=Recipes.views + 1))  # noqa
        res = await session.execute(select(Recipes.name, Recipes.time_to_create, Recipes.ingredients, Recipes.description).where(Recipes.name == name))  # noqa
        return res.fetchone()

    @app.post('/cookbook2/', response_model=SecondMonitor)  # 1 Пример POST endpointa # noqa
    async def books(name: FirstMonitor) -> Recipes:  # noqa
        """ Желательно указывать обязательно, т.к. по умолчанию FastAPI может тупить с типами данных и давать ошибки.
        SecondMonitor - модель валидации для возвращения данных
        FirstMonitor - модель валидации для получения данных """
        get_info = name.dict()  # noqa
        return get_info

    @app.post('/cookbook3')  # 2 Пример POST endpointa # noqa
    async def books(request: Request) -> Recipes:  # noqa
        data = await request.json()  # noqa
        return data
