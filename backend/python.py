class Information:
    ...
    check_test_count = """ Установка: pip install coverage
    Пример для Django:
        1. coverage run manage.py test  # Запускает тесты.
        2. coverage report  # Генерирует отчет.
        3. coverage html  # Создаст папку htmlcov в текущей директории, содержащую HTML-страницы, которые отображают покрытие кода.
    """  # Определение, на сколько приложение покрыто тестами.  # noqa
    ...
    pycharm = {
        ...
        "ALT + INSERT": "Сгенерировать код: [TEST | GETTER | SETTER].",
        "ALT + F7": "Посмотреть, где используется объект.",
        "CTRL + [D | Y | G | /]": "Выполнить действие для строки: [ДУБЛИРОВАТЬ | УДАЛИТЬ | ПЕРЕЙТИ | ЗАКОММЕНТИРОВАТЬ].",
        "CTRL + [BACK | DEL]": "Удаление символов до края слова в ЛЕВУЮ [ПРАВУЮ] сторону.",
        "CTRL + ALT + [V | M | T]": "Вынести выделенный текст в [ПЕРЕМЕННУЮ | МЕТОД | БЛОК].",
        "CTRL + ALT + C": "Выносит переменную в глобальное окружение.",
        "CTRL + ALT + L": "Быстрое автоматическое форматирование кода.",
        "CTRL + [ALT] + B": "Перейти к РОДИТЕЛЬСКОМУ [ДОЧЕРНЕМУ] объекту.",
        "CTRL + [ALT] + O": "Настройка импортов в файле: УДАЛИТЬ ЛИШНИЕ [ОПТИМИЗИРОВАТЬ].",
        "[CTRL] + ALT + [LEFT | RIGHT]": "Перемещение по ВКЛАДКАМ [ИСТОРИИ] проекта.",
        "CTRL + [SHIFT] + W": "Выделить БОЛЬШЕ [МЕНЬШЕ] кода.",
        "CTRL + [SHIFT] + E": "История работы с ФАЙЛАМИ [БЛОКАМИ].",
        "CTRL + [SHIFT] + N": "Найти КЛАСС [ФАЙЛ] в проекте.",
        "CTRL + [SHIFT] + Z": "Сделать шаг НАЗАД [ВПЕРЕД] в коде.",
        "CTRL + [SHIFT] + R": "Найти и заменить в ФАЙЛЕ [ПРОЕКТЕ].",
        "CTRL + [SHIFT] + F": "Найти в ФАЙЛЕ [ПРОЕКТЕ].",
        "CTRL + [SHIFT] + C": "Копировать ВЫДЕЛЕННЫЙ ТЕКСТ [ПУТЬ ДО ФАЙЛА].",
        "CTRL + [SHIFT] + V": "Буфер обмена: ВСТАВИТЬ [ОТКРЫТЬ ИСТОРИЮ].",
        "CTRL + [SHIFT] + H": "Посмотреть АКТИВНЫЙ [ДОЧЕРНИЙ] класс.",
        "CTRL + SHIFT + U": "Сменить регистр выделенных слов в windows.",
        "CTRL + SHIFT + [TOP | DOWN]": "Перенос строки [ВВЕРХ | ВНИЗ].",
        "CTRL + SHIFT + [Y | X]": "Перевести и [ПОКАЗАТЬ | ЗАМЕНИТЬ] выделенный текст. [Translation plugin]",
    }  # Сочетания кнопок для PyCharm. # noqa


class Python:
    """ Класс, в котором хранятся некоторые небольшие шаблоны. Сложных вещей тут нет. """
    @staticmethod  # Аннотации типов.
    def annotations():
        from typing import TypedDict, TypeVar, NewType, Union, Any, Generator, AsyncGenerator  # noqa

        # TypedDict. Ответ: user_dict: User = {"id": 1, "name": "name"}. Еще можно так: dict[str, str]. # noqa
        class User(TypedDict):  # Аннотирует словарь, т.е. {"id": 1, "name": "name"} будет проверяться на корректность. # noqa
            id: int
            name: str

        # TypeVar. Из списка [1, '2'] будет возвращать int, str...  # noqa
        T = TypeVar('T')  # Аннотирует индивидуально поданные данные.  # noqa
        def my_func(i: list[T]) -> list[T]:  # noqa
            yield i  # Будет возвращать и указывать тип каждого объекта.

        def gen() -> Generator[str, None, None]:  # Генераторы: [YieldType, SendType, ReturnType] - Возвращает соответствующие данные. # noqa
            yield "str"

        IntStr = NewType("IntStr", str)  # noqa
        new: IntStr = IntStr("10")  # NewType. Создает новый тип данных. Если проще: будет требовать подавать данные в виде конкретного класса и его типа. # noqa
        union: Union[str, int, float] = "1" or 1 or 1.0  # Union. Позволяет указать, что данные могут быть одним из нескольких типов. # noqa
        any_type: Any = "1" or 2 or 3.0 or [] or {}  # Any. Отключает проверку типа вообще, поэтому НЕБЕЗОПАСНО. # noqa
    ...
    def lambdas():  # noqa
        ...
        if a := 1 + 2:  # Дополнительная штука: "выражение присваивания". Позволяет задать условие переменной прямо при вычислении.
            print(a)
    ...
    def decorators():  # noqa
        ...
        good_decorators = {
            ...
            "@classmethod": "Ставится на методы, которые принимают свой класс аргументом (полностью класс а не его содержимое), его первый аргумент: cls, например cls.name",  # noqa
            "@hybrid_method": "Означает, что атрибут имеет различное поведение, определенное на уровне класса и на уровне экземпляра",
            "@abstractmethod": "Метод для класса, который наследуется от ABC, который не будет создаваться как объект, а только наследоваться другими классами.",
            "@property": "Нужен при сокрытии данных (__val параметров). Имеет и функцию и декоратор (getter, setter, deleter).",
        }
    ...
    def async_multiprocessing():  # noqa
        ...
        apply_async - Не ждет завершения задач. Не соблюдает порядок возвращаемых данных.  Можно использовать разные функции ко всем значениям.
        map_async - Блокирует код и ждет завершение всех задач. Соблюдает порядок возвращаемых данных. Можно использовать одну функцию ко всем значениям.
        """  # noqa
        def threading():  # Потоки - хорошо подходят для IO-bound операций. Число соединений: очень большое. # noqa
            """ Особенности работы с потоками:
            + Общие ресурсы ПК.
            + Требуют меньше ресурсов, чем процессы.
            + Хорошо работают с IO-bound операциями (input/output), т.е. сетевые запросы, чаты и т.д. 
            - Небезопасны (могут в 1 момент изменить какой то файл...), требуют доп. синхронизации: semaphore, spinlocks, крит. секций (это ресурсы).
            - Могут блокировать друг друга при попытке захватить процесс.
            - Ограничены GIL'ом: только 1 поток может выполняться в настоящее время.
            """  # Плюсы и минусы. # noqa

            def classic():  # Использование потоков на прямую. # noqa
                import threading

                thread = threading.Thread(target=my_func)  # Создает новый поток. # noqa
                thread.start()  # Запускает поток в работу.
                thread.join(timeout=5)  # Блокирует поток до тех пор, пока он не выполнит свою работу. Можно указать время ожидания выполнения потока.

            def threadpool():  # Использование потоков через ThreadPool. Устаревший вариант. # noqa
                from multiprocessing.pool import ThreadPool

                with ThreadPool(processes=999) as pool:  # Создается pool, открываются сами методы '.start()', '.join()'.
                    threads = [pool.apply_async(my_func, (i,)) for i in range(9)]  # создаем задачи для потоков: функция, аргументы. # noqa
                    waits = [thread.wait() for thread in threads]  # Ждем, пока выполнятся все потоки. # noqa
                    results = [wait.get() for wait in waits]  # Получаем результаты работы потоков. # noqa

            def threadpoolexecutor():  # Использование потоков через ThreadPoolExecutor. Современный вариант. # noqa
                from concurrent.futures import ThreadPoolExecutor, as_completed

                with ThreadPoolExecutor(max_workers=999) as executor:  # Создается pool, открываются сами методы '.start()', '.join()'.
                    threads = [executor.submit(my_func, i) for i in range(9)]  # создаем задачи для потоков: функция, аргументы. # noqa
                    results = [thread.result() for thread in as_completed(threads)]  # Ждем, пока выполнятся все потоки и получаем результаты. # noqa
                    results = [executor.map(my_func, [1, 2])]  # Метод map() отдает результаты в том же порядке. # noqa

        def processing():  # Процессы - хорошо подходят для вычислительных операций. Число соединений: зависит от кол-ва ядер процессора. # noqa
            """ Особенности работы с процессами:
            + Работают реально параллельно(блокируются только в коде ядра).
            + Используют все ресурсы процессора.
            + Безопасны (простая синхронизация).
            + Отдельные программы со своими ресурсами (полностью изолированы друг от друга). 
            - Нет общей памяти и т.д.
            - Нужна синхронизация для сложных вычислений.
            - Затратны по ресурсам (под каждый процесс отдельная копия интерпретатора со своими ресурсами...).
            - Зависит от количества ядер процессора.
            """  # Плюсы и минусы. # noqa

            def classic():  # Использование процессов на прямую. # noqa
                from multiprocessing import Process, Queue, Pipe, Manager  # noqa

                def my_queue(queue, a):  # noqa
                    two_a = a + a
                    queue.put(two_a)

                queue = Queue()  # Позволяет передавать данные между процессами.
                process_1 = Process(target=my_queue, args=(queue, 2))  # Создает новый процесс. # noqa
                process_1.start()  # Запускает процесс в работу.
                queue.get()  # Для чтения и удаления объекта из очереди.
                process_1.join()  # Заставляет основной процесс ожидать завершения дочернего процесса.

                def my_pipe(conn):  # noqa
                    conn.send([42, None, 'hello'])
                    conn.close()

                parent_conn, child_conn = Pipe(duplex=True)  # Создает две соединенные конечные точки для обмена данными между процессами. duplex - двусторонняя. # noqa
                process_2 = Process(target=my_pipe, args=(child_conn,))
                process_2.start()  # Запускает процесс в работу.
                parent_conn.recv()  # Читает и получает данные из соединения, блокирует выполнение до тех пор, пока не будут получены данные из соединения.
                process_2.join()  # Заставляет основной процесс ожидать завершения дочернего процесса.

            def pool():  # Использование процессов через Pool. # noqa
                from multiprocessing import Pool, cpu_count

                with Pool(processes=cpu_count()) as pool:  # Создается pool, открываются сами методы '.start()', '.join()'. # noqa
                    processes = [pool.apply_async(my_func, (i, i * 2)) for i in range(10)]  # создаем задачи для процессов: функция, аргументы. # noqa
                    waits = [proc.wait() for proc in processes]  # Ждем, пока выполнятся все процессы. # noqa
                    results = [res.get() for res in waits]  # Получаем результаты работы процессов. # noqa

        def asynchronous():  # noqa
    ...
    def csv_file():  # noqa
        ...
        with open("file.csv", 'a', encoding='utf-8') as csvfile:  # Открывает .csv файл и добавляет в него новые строчки. Параметр newline="" - так же уберет '\n'.
