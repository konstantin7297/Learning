...
class SQLAlchemy:
    ...
    @staticmethod
    def database_urls():
        """ Основные подключаемые штуки к БД. """
        DATABASE_URL = "sqlite+aiosqlite:///testdb.db"  # Асинхронное подключение к SQL для тестов, pip install aiosqlite # noqa
        DATABASE_URL = "sqlite+sqlite3://login:password@ip/db.db"  # Синхронное подключение к SQL # noqa
        DATABASE_URL = "sqlite+aiosqlite://login:password@ip/db.db"  # Асинхронное подключение к SQL, pip install aiosqlite # noqa
    ...
    def async_database():
        ...
        import session  # fake import # noqa
        import asyncio
        from typing import Dict, List
        from typing_extensions import Annotated
        from sqlalchemy import ForeignKey, UniqueConstraint, Table, Column, select, insert, update, delete, func  # noqa
        ...
        async_session = async_sessionmaker(bind=async_engine, class_=AsyncSession)  # expire_on_commit - истечение срока действия объекта после коммита. # noqa
        session = async_session()  # noqa

        # Тут показан принцип составления аннотаций типов в современном стиле. Применение: id: Mapped[int_apk], а это: = relationship(...) можно уже в самой таблице.
        int_apk = Annotated[int, mapped_column(autoincrement=True, primary_key=True)]  # noqa
        str_u = Annotated[str, mapped_column(unique=True)]  # noqa
        user_fk = Annotated[int, mapped_column(ForeignKey("users.id", ondelete="CASCADE"))]  # noqa
        user_fku = Annotated[int, mapped_column(ForeignKey("users.id", ondelete="CASCADE"), unique=True)]  # noqa
        user_fpk = Annotated[int, mapped_column(ForeignKey("users.id", onupdate="CASCADE"), primary_key=True)]  # noqa
        address_fpk = Annotated[int, mapped_column(ForeignKey("addresses.id", ondelete="CASCADE"), primary_key=True)]  # noqa

        class Base(MappedAsDataclass, DeclarativeBase):  # Универсальный класс, от которого наследуются остальные и сразу имеют одинаковые колонки id
            __allow_unmapped__ = True  # Нужна для аннотаций под mypy в старых версиях (до SQLAlchemy 2.0).
            id: Mapped[int_apk] = mapped_column(init=False)  # init - значит это значение не часть __init__, и не создается при инициализации класса. # noqa

            @classmethod
            @declared_attr  # noqa
            def __tablename__(cls) -> str:  # Название таблицы = названию класса в нижнем регистре. Заменяет: __tablename__ = "users", и ее можно удалить # noqa
                return cls.__name__.lower().join("s")
        ...
        def connections():  # noqa
            """ Тут расписаны связи и их нюансы. Для самой связи достаточно, что бы был указан ForeignKey, он обеспечит соединение всех данных, вручную в
            список child ничего не нужно добавлять. """

            def one_to_one():  # noqa
                """ Связь один к одному, user_id обязательно должен быть уникальным, что бы несколько таких записей не ссылались на 1 child, т.к. это уже будет связь 1к9. """
                str_u = Annotated[str, mapped_column(unique=True)]  # noqa
                user_fku = Annotated[int, mapped_column(ForeignKey("users.id"), unique=True, nullable=False, onupdate="SET NULL")]  # unique, потому-что связь строго 1к1. # noqa

                class User(Base):  # noqa
                    __tablename__ = "users"
                    name: Mapped[str_u]
                    child: Mapped["Address"] = relationship("Address", backref="parent", uselist=False, cascade="delete", lazy="joined")

                class Address(Base):  # noqa
                    __tablename__ = "addresses"
                    email: Mapped[str_u]
                    user_id: Mapped[user_fku]
                    # parent: Mapped["User"] = relationship("User", back_populates="child")  # Строка нужна только если child сделан через back_populates для обратной связи # noqa

            def one_to_many():  # noqa
                """ Связь один ко многим. Тут уже ForeignKey не должен быть уникальным, т.к. нам нужна связь 9к1. """
                str_u = Annotated[str, mapped_column(unique=True)]  # noqa
                user_fk = Annotated[int, mapped_column(ForeignKey("users.id"), nullable=False, onupdate="SET NULL")]  # noqa

                class User(Base):  # noqa
                    __tablename__ = "users"
                    name: Mapped[str_u]
                    child: Mapped[List["Address"]] = relationship("Address", backref="parent", uselist=True, cascade="delete", lazy="selectin")
                ...
                def one_table():  # noqa
                    ...
                    class User(Base):  # noqa
                        ...
                        following: Mapped[List["User"]] = relationship(
                            ...
                            primaryjoin="User.id == associations.c.following",
                            secondaryjoin="User.id == associations.c.follower",
                        ...
                        follower: Mapped[List["User"]] = relationship(
                            "User",
                            secondary="associations",
                            primaryjoin="User.id == associations.c.follower",
                            secondaryjoin="User.id == associations.c.following",
        ...
        class User(Base):  # Таблица для примера методов запросов # noqa
            """ Тут в примерах запросах используется session.execute(). Особенность: просто выполняет заранее подготовленный запрос в формате SQL, так же более
            безопасен, т.к. выполняет только 1 запрос до ';'. """
            ...
            @staticmethod
            async def insert(name: str, age: int):
                result1 = await session.execute(insert(User).returning(User.id).values(name=name, age=age))
                await session.commit()
                print(result1.fetchone())  # return (1,)

            @staticmethod
            async def update(name: str, age: int):
                result1 = await session.execute(update(User).where(User.name == name).returning(User.id).values(age=age))
                await session.commit()
                print(result1.fetchone())  # return (1,)

            @staticmethod
            async def select(user_id: int):
                result1 = await session.execute(select(User.id).where(User.id == user_id))  # type: ignore
                print(result1.fetchone())  # return (1,)

            @staticmethod
            async def delete(user_id: int):
                result1 = await session.execute(delete(User).returning(User.id).where(User.id == user_id))
                await session.commit()
                print(result1.fetchone())  # return (1,)

            @staticmethod
            async def bulk_methods():
                async with session.begin():  # То же самое, что и: await session.commit()
                    await session.bulk_save_objects([User(name="kolya", age=4), User(name="petya", age=5)])  # Если они уже в таблице, вызывается UPDATE
                    await session.bulk_insert_mappings(User, [{"name": "kolya", "age": 5}, {"name": "petya", "age": 6}])
                    await session.bulk_update_mappings(User, [{"id": 1, "name": "kolya"}, {"id": 2, "name": "petya"}])

            @staticmethod
            async def sub_request():
                sub_q = select(func.max(User.age)).select_from(User).subquery()  # return max age
                result = await session.execute(select(User.id).where(User.age == sub_q))  # select_from - для поиска из саб. запроса # noqa
                print(result.fetchone())  # return (1,)

            @staticmethod
            async def big_request():
                stmt = select(func.sum(Book.count), Author.name).where(Book.author_id == Author.id).group_by(Author_id).order_by(func.sum(Book.count).desc())  # noqa
                result = await session.execute(stmt)  # noqa
                print(result.fetchall())  # return [(9, Lena),]

            @staticmethod
            async def another():
                User.id.label('user_id')  # label: SELECT "id" AS "user_id" FROM "users" # noqa
                ...
                user.one_to_more.append(character)  # Примерная схема добавления связи. Для списка: .extend([my_mail, my_mail2...])
