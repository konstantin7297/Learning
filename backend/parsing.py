class Selenium:
    """ Парсинг через Selenium: pip install selenium, pip install selenium-wire - дополнение для selenium, позволяет добавить доп. настройки. """
    from getpass import getuser   # noqa
    from selenium import webdriver  # Сам браузер  # noqa
    from selenium.webdriver import ActionChains  # Нужен для создания цепочки действий. # noqa
    ...
    def __init__(self, main_url: str, timeout: int = 5):
        ...
        if sys.platform == "win32":  # Подключается к профилю пользователя в Windows. Для параллельных запусков не стоит указывать 1 пользователя. # noqa
