...
class Product(models.Model):
    ...
    class Meta:  # Дополнительные параметры для работы модели таблицы БД: сортировка полей, имя таблицы, разрешения и т.д.
        ...
        db_table = "products"  # Полностью заменяет имя таблицы для обращения к ней.
        db_table_comment = "table with products"  # Комментарий для всей таблицы.
        using = "products"  # Указывает, какую базу данных использует эта модель, если в проекте их несколько. # noqa
...
class Order(models.Model):
    ...
    class Meta:  # Дополнительные параметры для работы модели таблицы БД: сортировка полей, имя таблицы, разрешения и т.д.
        ...
        permissions = [("can_deliver_pizzas", "Can deliver pizzas")]  # Дополнительные разрешения для таблицы.
        constraints = []  # Ограничения таблицы. Тут проверки условий, уникальность значений и другие параметры, например: 'models.CheckConstraint()'. # noqa
        db_table = "orders"  # Полностью заменяет имя таблицы для обращения к ней.
        db_table_comment = "table with orders"  # Комментарий для всей таблицы.
        using = "orders"  # Указывает, какую базу данных использует эта модель, если в проекте их несколько. # noqa
...
@receiver(pre_save, sender=Product)  # Позволяет поймать сигнал 'pre_save' - перед сохранением объекта от 'Product'. # noqa
def my_signal(sender, instance, **kwargs):  # Можно обработать дополнительные действия при поимке сигнала. # noqa
    """ Функция для обработки объекта к модели Product перед его сохранением. Signals - синхронные, так что нужно работать с ними осторожно. """
    sender  # Это сама модель таблицы базы данных. # noqa
    instance.name = 'new value'  # instance - сам объект к базе данных.
