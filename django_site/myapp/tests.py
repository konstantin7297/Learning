from rest_framework.test import APIClient, RequestsClient, APIRequestFactory  # Через него тестируется DRF. # noqa
from asgiref.testing import AsyncClient  # Через него тестируется асинхронный код. # noqa
from django.contrib.auth.models import User  # noqa
from django.test import TestCase, LiveServerTestCase  # LiveServerTestCase - Позволяет тестировать фулл приложение через selenium. # noqa
from django.dispatch import Signal  # noqa
...
class RestFrameworkTestCase(TestCase):
    ...
    def setUp(self):  # noqa
        self.factory = RequestsClient()  # Через него тестируется API от REST Framework.  # noqa
...
order_saved = Signal(providing_args=["order"])  # Для примера тестирования сигналов.


class SignalsTest(TestCase):
    def test_order_saved_signal(self):
        """ Тестирует, что вызывается сигнал при создании объекта в модели 'Order'. """
        self.signal_called = False

        def order_saved_signal_handler(sender, **kwargs):  # noqa
            self.signal_called = True

        order_saved.connect(order_saved_signal_handler)

        order = Order(name="Test Order")  # noqa
        order.save()
