...
from django.contrib.auth.decorators import login_required, permission_required, user_passes_test  # noqa
from django.contrib.auth.mixins import LoginRequiredMixin, PermissionRequiredMixin, UserPassesTestMixin  # noqa
from django.contrib.syndication.views import Feed  # noqa
from django.core.cache import cache  # noqa
from django.core.files.storage import FileSystemStorage  # noqa
from django.db import transaction  # noqa
from django.db.models import Avg, Max, Min, Count, F, Q, Subquery  # noqa
...
from rest_framework.authentication import SessionAuthentication, BasicAuthentication  # noqa
from rest_framework.parsers import MultiPartParser  # noqa
...
from rest_framework.mixins import ListModelMixin  # noqa
from rest_framework.generics import GenericAPIView, ListCreateAPIView  # noqa
from rest_framework.decorators import api_view, action  # noqa
from rest_framework.response import Response  # noqa
from rest_framework.request import Request  # noqa
from drf_spectacular.utils import extend_schema, OpenApiResponse  # noqa
from django.http import HttpResponse, HttpResponseNotFound, Http404, HttpRequest, HttpResponsePermanentRedirect, HttpResponseRedirect  # noqa
from django.views.generic import TemplateView, ListView, DetailView, CreateView, UpdateView, DeleteView  # noqa
from django.utils.translation import gettext, ngettext_lazy  # noqa
...
class DatabaseView(View):  # Можно использовать сессии с ленивой загрузкой, что бы не загружать лишние данные.
    ...
    @transaction.atomic  # Делает функцию атомарной, т.е. либо выполняется все, либо ничего. Так же оптимизирует запросы: отправляет все изменения в БД одним запросом.
    def get(self, request: HttpRequest) -> HttpResponse:  # noqa
        result = get_object_or_404(Product, pk=1)  # Вернет найденную запись, либо выкинет 404 исключение. # noqa
        result = Product.select_for_update(...)  # Вернет найденную запись и заблокирует ее от параллельных изменений, пока происходит эта транзакция. # noqa
        result, created = Product.objects.get_or_create(...)  # Найдет или создаст новую запись без вызова '.save()'. result - запись, created - статус создания. # noqa
        subquery = Subquery(Product.objects.all())  # Вспомогательный запрос. Делает 1 запрос в БД, что бы подставлять эти данные во 2 запрос. # noqa

        Order.objects.prefetch_related("images")  # Оптимизация загрузки связей ManyToMany. # noqa
        Order.objects.select_related("user")  # Оптимизация загрузки связей OneToOne. # noqa
        Product.objects.bulk_create([Product(...) for _ in range(9)])  # Создает и сразу сохраняет список записей сразу без вызова '.save()'. # noqa
        Product.objects.bulk_update([Product(...) for _ in range(9)])  # Обновляет и сразу сохраняет список записей сразу без вызова '.save()'. # noqa

        Product.objects.all()  # Возвращает все записи из таблицы. # noqa
        Product.objects.create(...)  # Создает и сразу сохраняет запись сразу без вызова '.save()'. # noqa
        Product.objects.delete()  # Удаляет все полученные отфильтрованные данные. # noqa
        Product.objects.distinct()  # Вернет только уникальные записи. # noqa
        Product.objects.first()  # Вернет только первую найденную запись. # noqa
        Product.objects.last()  # Вернет только последнюю найденную запись. # noqa
        Product.objects.exists()  # Вернет True / False, будут ли найдены какие то записи по данному запросу. # noqa
        Product.objects.using('db_name')  # Указывает, к какой БД делать запрос, если их используется несколько. # noqa
        Product.objects.order_by('price')  # Сортирует записи по заданным полям. # noqa
        Product.objects.order_by('price')  # Сортирует записи по заданным полям. # noqa
        Product.objects.only("name")  # only - загружает только указанные поля, а остальные нет. # noqa
        Product.objects.defer("description")  # defer - загружает указанные поля в ленивом режиме. Т.е. только при обращении к ним. # noqa
        Product.objects.values('name', 'price')  # Возвращает значения полей в виде словарей. # noqa
        Product.objects.values_list('name', 'price')  # Возвращает значения полей в виде списков. flat=True - распакует данные в 1 список. # noqa
        Product.objects.get(name='...')  # Возвращает первый подходящий объект или вызывает 'DoesNotExist'. # noqa
        Product.objects.update(discount=10, price=F('price') * 1.10)  # Обновление всех отфильтрованных данных. F - обращение к полю не загружая его в память. # noqa
        Product.objects.annotate(all_products=Count("name", default=0))  # Задает имя полученным данным для обращения: 'Product.all_products'. # noqa

        Product.objects.aggregate(  # Занимается вычислением каких то значений и их возвращением. # noqa
            medium=Avg("price"),  # Вернет СРЕДНЕЕ найденное значение. # noqa
            maximum=Max("price"),  # Вернет МАКСИМАЛЬНОЕ найденное значение. # noqa
            minimum=Min("price"),  # Вернет МИНИМАЛЬНОЕ найденное значение. # noqa
            count=Count("id"),  # Вернет КОЛИЧЕСТВО найденных значений. # noqa
            array=ArrayAgg("id"),  # Вернет СПИСОК найденных значений: '[1, 2, 3, 4]'. # noqa
            string=StringAgg("id"),  # Вернет СТРОКУ скомбинированных найденных значений: '1;2;3;4'. # noqa
        )  # noqa

        Product.objects.filter(  # Возвращает подходящие по условиям отфильтрованные записи. Через '__' можно обращаться к особым функциям или связанным объектам. # noqa
            Q(price__lt=20) | ~Q(price__gt=100),  # 'Q' - Хранит Истинное(верное) значение. '~Q' - Хранит Ложное(неверное) значение. # noqa
            Q(price__lt=20) | Q(price__gt=100),  # '|' - оператор ИЛИ. # noqa
            Q(price__lt=20) & Q(price__gt=100),  # '&' - оператор И. Их можно просто через запятую. # noqa
            price__gte=F('price') * 1.10,  # 'F' - Позволяет работать с полем из БД не загружая его в память python. # noqa
            name__contains='тя',  # contains[icontains] - Определяет, что в поле есть указанная строка С[БЕЗ] учетом регистра. # noqa
            name__exact='костя',  # exact[iexact] - Определяет точное совпадение поля со значением С[БЕЗ] учетом регистра. # noqa
            name__startswith='ко',  # startswith[istartswith] - Определяет, что поле начинается со значения С[БЕЗ] учетом регистра. # noqa
            name__endswith='стя',  # endswith[iendswith] - Определяет, что поле заканчивается на значение С[БЕЗ] учетом регистра. # noqa
        )  # Через '__' можно обращаться к особым функциям или связанным объектам. # noqa

        Product.objects.exclude(  # Возвращает НЕ подходящие условиям отфильтрованные записи. Через '__' можно обращаться к особым функциям или связанным объектам. # noqa
            price__gte=5,  # Поле 'price' БОЛЬШЕ ИЛИ РАВНО указанному значению. # noqa
            price__gt=5,  # Поле 'price' БОЛЬШЕ, чем заданное значение. # noqa
            price__lte=5,  # Поле 'price' МЕНЬШЕ ИЛИ РАВНО указанному значению. # noqa
            price__lt=5,  # Поле 'price' МЕНЬШЕ, чем заданное значение. # noqa
            price__in=[1, 2, 3],  # Поле 'price' содержится в списке. # noqa
            date__time=datetime.time(14, 30),  # date|time|year|month|day|hour|minute|second... - Фильтруют поле по заданному значению. # noqa
        )  # Через '__' можно обращаться к особым функциям или связанным объектам. # noqa

        double_result = Product.objects.using("db1") | Product.objects.using("db2")  # Так делается запрос сразу к 2 базам данных. # noqa
...
class ProductModelViewSet(ModelViewSet):  # Класс подключается в основном файле 'urls.py' добавляет целую API по разным операциям над моделью 'Product'.
    ...
    def retrieve(self, *args, **kwargs):  # Этот метод API для продуктов настраивает страницу для возвращения 1 конкретного продукта. OpenApiParameter('page', exclude=True) - выключит поле #
